(ns cesium-xr.main
  "Kotodama WASM entrypoint for the Cesium_xr clean-room actor (L4) — Clojure port.
  ...
  facts (`cesium_xr.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cesium_xr")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cesium_xr.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cesium_xr-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cesium_xr-compat/ready)
